      ...
      try {
        ...
        if (user) {
          ...
          setWishlistProductIds(
            new Set(wishlistRes.data.map((item) => item.product_id))
          );
          setCartProductIds(
            new Set(cartRes.data.map((item) => item.product_id))
          );
        }
      } catch (err) {
        ...
        <Grid container spacing={3}>
          {products.length === 0 ? (
            <Typography
              variant="body1"
              align="center"
              sx={{ width: "100%", color: theme.secondaryText, mt: 5 }}
            >
    ...
    <Card
      sx={{
        ...
        "&:hover": {
          ...
          boxShadow: "0 8px 20px rgba(93, 64, 55, 0.15)",
        },
      }}
    >
        ...
        <Link to={`/product/${product.id}`} style={{ textDecoration: "none" }}>
          <CardMedia
            ...
            image={
              ...
              "https://placehold.co/400x400/F5EFE6/5D4037?text=No+Image"
            }
            alt={product.name}
          />
        </Link>
      ...
      >
        ...
        >
          <Typography
            ...
            variant="h6"
            component="div"
            sx={{ color: theme.primaryText, flexGrow: 1 }}
          >
            {product.name}
          </Typography>
        </Link>
        ...
        >
          ...
          <IconButton
            onClick={(e) => onToggleCart(e, product)}
            sx={{
              border: `1px solid ${isInCart ? "#4CAF50" : theme.lightBorder}`,
            }}
          >
            {isInCart ? (
              <RemoveShoppingCart sx={{ color: theme.secondaryText }} />
            ) : (
              <AddShoppingCart sx={{ color: theme.accent }} />
            )}
          </IconButton>
        </Box>
      </CardContent>
